...
def main():
    runs = {
        "Position impedance":       run("impedance"),
        "PID + force governor":     run("pid_force"),
        "Tip-only MPC":             run("mpc"),
        "Constrained joint/task MPC": run("mpc_kalman"),
    }
    print("\n" + "="*78)
    ...
    colors = {
        "Position impedance": "tab:red",
        "PID + force governor": "tab:orange",
        "Tip-only MPC": "tab:blue",
        "Constrained joint/task MPC": "tab:green",
    }
    for name, L in runs.items():
        c = colors[name]
