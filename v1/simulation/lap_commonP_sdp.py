...
The polytope here is the operational-space inertia spread in the exact-ZOH
input matrix B_d(rho) = [-0.5 Lambda^{-1} dt^2; -Lambda^{-1} dt], scaled over a
4x range, matching the 'gamma vs inertia scale' surrogate reported by
lap_hinf_design.py.
...
def main():
    ...
    scales = (0.5, 1.0, 2.0)
    B_vertices = [
        np.vstack([-0.5 * dt * dt * (Lam_inv * s), -(Lam_inv * s) * dt])
        for s in scales
    ]
