...
This script:
  ...
  5. reports the force budget  F_max = gamma * wbar  (Eq. 15a);
...
def main():
    ...
    # channel (E = B). The performance output weights tracking error AND the
    # applied tip force with a *real* unit weight (the force is the safety
    # quantity), so ||T_{w->z}||_inf <= gamma  =>  peak force <= gamma * wbar.
    E = B.copy()
    ...
    out["Hinf"] = dict(gamma_min=float(gamma),
                       ...
                       wbar_N=wbar,
                       F_max_N=float(gamma*wbar))

    # --- robustness over operational-space inertia spread (true LPV param) ---
    gammas = {}
    for scale in (0.5, 1.0, 2.0):
        Bs = np.vstack([np.zeros((3, 3)), -(Lam_inv*scale)*p.dt_mpc])
    ...
    print(f"  force budget F_max = gamma_min * wbar = {out['Hinf']['F_max_N']:.3f} N "
          f"(wbar = {wbar:.1f} N)")
    ...
    print("  (a single common-Lyapunov gamma over the box needs an SDP/LMI "
          "solver, e.g. cvxpy — see Section 6.2; here we report per-vertex.)")
