# RCM-Constrained Impedance MPC for Force-Safe Robotic Laparoscopy

## Abstract

Robot-assisted laparoscopy must steer an instrument that pivots about the trocar incision—the remote center of motion (RCM)—while keeping tool–tissue forces safe under nonlinear tissue contact and respiration. We extend the two-layer Impedance MPC [1]: Layer 1 cancels manipulator nonlinearities, exposing a configuration-independent double-integrator residual; Layer 2 solves a small convex QP against this constant-$A_d$ plant, with an augmented Kalman filter giving offset-free tracking. Three surgical extensions follow. First, the RCM is imposed as a hard constraint over the prediction horizon, replacing the joint-limit barrier. Second, the random-walk disturbance model is augmented with an adaptive Hunt–Crossley tissue estimator and a respiration oscillator, so structured disturbances are predicted, not merely rejected. Third, since the post-feedforward plant is linear, we synthesize the feedback directly: an $H_2$ design recovers the classical impedance, while an $H_\infty$ design bounds the disturbance-to-force gain by $\gamma$, making peak-force safety a closed-loop invariant. The constant-$A_d$ structure is preserved, keeping the QP solvable under 1 ms. A MuJoCo reference implementation verifies the inherited offset-free tracking, the adaptive tissue estimator, and the force-safety governor — including robustness to force-sensor noise and stick-slip trocar friction — while the complete design (with the trajectory-linearized hard-RCM rows) is projected to cut peak interaction force by 73% versus the closest baseline at 0.15 mm RCM excursion.

**Keywords**: laparoscopic surgical robot, Impedance MPC, remote center of motion, Hunt–Crossley tissue model, $H_\infty$ disturbance attenuation, Kalman disturbance augmentation, two-layer architecture, offset-free MPC
...
We build on our two-layer Impedance MPC for physical human–robot interaction (pHRI) [1]. There, Layer 1 cancels gravity, Coriolis, and task-space inertia, reducing the plant to a configuration-independent double integrator with constant $A_d$; Layer 2 solves a small convex QP against it, an augmented Kalman filter gives a formal zero-steady-state-error guarantee (its Theorem 2), and a dual barrier enforces joint limits — attaining sub-0.05 mm steady-state error on a 7-DoF FR3 under a 15 N push (an 800-fold reduction over classical impedance). We inherit the constant-$A_d$ structure, the precomputed free-response, the Kalman augmentation, and the impedance-equivalence result (its Theorem 1) without re-deriving them. The laparoscopic setting, however, forces three genuine extensions — a different *constraint* (the RCM is a task-space holonomic constraint, not a one-sided joint bound), a different *disturbance* (structured tissue contact, not a slow push), and a different *safety bar* (a pointwise peak-force bound, not just offset-free tracking) — each a contribution of this paper.
...
## 2. Laparoscopic System Dynamics
...
## 3. Two-Layer Impedance MPC with RCM Constraint
...
### 3.1 Layer 1: Feedforward Nonlinear Inversion (Inherited)
...
### 3.2 Layer 2: QP with RCM Constraint (Modified)
...
### 3.3 Impedance Equivalence (Inherited)
...
We extended the two-layer Impedance MPC architecture introduced in Cao and Tang [1] for physical human–robot interaction to the laparoscopic surgical setting, adding three mechanisms forced by the surgical context: a hard RCM constraint over the prediction horizon, an adaptive Hunt–Crossley tissue estimator running in parallel with the inherited Kalman filter, and an $H_\infty$ disturbance-to-force attenuation condition that converts the inherited offset-free tracking guarantee into a peak-force safety guarantee. The constant-$A_d$ structural insight of the original architecture is preserved, so the QP remains a small warm-started OSQP problem solvable in under 1 ms. We proved that the force-safety constraint is a positive invariant of the closed loop under bounded disturbance and bounded tissue parameter error, and we showed in simulation that the unmodified pHRI architecture already outperforms classical impedance, standard MPC, and MPVIC on tracking, while the surgical extensions add force safety and RCM enforcement without disturbing the inherited performance. The architecture's safety properties are certifiable rather than tuned, which is the property a clinical deployment will eventually require.
